...
class NewsMixins:
    def get_context_data(self, **kwargs):
        ...
        context['most_viewed'] = Post.objects.filter(
            published_at__isnull=False, status='active').order_by('-views_count').first()
        context['top_posts'] = Post.objects.filter(
            published_at__isnull=False, status='active').order_by('-views_count')[:3]
        context['weekly_posts'] = Post.objects.filter(
            published_at__isnull=False, status='active', published_at__gte=one_week_ago).order_by('-views_count')[:5]
        context['random_posts'] = Post.objects.filter(
            published_at__isnull=False, status='active').order_by('?')[:5]
        return context


class HomeView(NewsMixins, ListView):
    model = Post
...
class PostListView(NewsMixins, ListView):
    model = Post
...
class PostDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        obj.save()
        context = super().get_context_data(**kwargs)
        context["comments"] = obj.comment_set.all().order_by(
            '-created_at')[:10]
